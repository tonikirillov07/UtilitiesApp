...
public class RecordsWriter {
    public static void addAgent(AgentRecord agentRecord, String databasePath){
        try {
            String add = "INSERT INTO " + Agents.TABLE_NAME + "(" + Agents.NAME_ROW + "," + Agents.SURNAME_ROW + "," + Agents.PERSONAL_CODE_ROW + "," + Agents.ADDRESS_ROW + "," + Agents.TELEPHONE_ROW + ") VALUES(?,?,?,?,?)";
    ...
    public static void addCondole(CondolesRecord condolesRecord, String databasePath){
        try {
            String add = "INSERT INTO " + Condoles.TABLE_NAME + "(" + Condoles.NUMBER_ROW + "," + Condoles.OWNER_NAME_ROW + "," + Condoles.ROOMS_NUMBER_ROW + "," + Condoles.PEOPLE_NUMBER_ROW + ") VALUES(?,?,?,?)";
            PreparedStatement preparedStatement = Objects.requireNonNull(DatabaseService.getConnection(databasePath)).prepareStatement(add);
